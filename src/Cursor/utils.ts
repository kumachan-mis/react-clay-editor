...
export function cursorPropsToState(props: Props, state: State): State {
  ...
  if (charElement) {
    ...
    if (charRect.top - rootRect.top < 0) charElement.scrollIntoView(true);
    else if (charRect.bottom - rootRect.top > rootRect.height) charElement.scrollIntoView(false);
    ...
    if (rect.top - rootRect.top < 0) element.scrollIntoView(true);
    else if (rect.bottom - rootRect.top > rootRect.height) element.scrollIntoView(false);
