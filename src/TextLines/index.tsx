...
const Indent: React.FC<IndentProps> = (props) => {
  ...
  return (
    <span className={constants.className} style={constants.style(props.indent.length)}>
      {[...props.indent].map((char: string, index: number) => {
        const charClassName = TextLinesConstants.char.className(props.lineIndex, index);
        return (
          <span key={index} className={`${constants.pad.className} ${charClassName}`}>
            {" "}
          </span>
        );
      })}
