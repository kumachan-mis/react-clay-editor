...
const TaggedLink: React.FC<NodeProps<TaggedLinkNode>> = ({
  ...
  return (
    <EmbededLink
      ...
      anchorProps={(hover) => taggedLinkVisual?.anchorProps?.(linkName, hover)}
    >
      {[...facingMeta].map((char, index) => (
        <Char key={first + index} lineIndex={lineIndex} charIndex={first + index}>
          {cursorOn ? char : ''}
...
const BracketLink: React.FC<NodeProps<BracketLinkNode>> = ({
  ...
  return (
    <EmbededLink
      cursorOn={cursorOn}
      forceActive={linkForceActive}
      anchorProps={(hover) => bracketLinkVisual?.anchorProps?.(linkName, hover)}
    >
      {[...facingMeta].map((char, index) => (
        <Char key={first + index} lineIndex={lineIndex} charIndex={first + index}>
          {cursorOn ? char : ''}
...
const Hashtag: React.FC<NodeProps<HashtagNode>> = ({
  ...
  return (
    <EmbededLink
      cursorOn={cursorOn}
      forceActive={linkForceActive}
      anchorProps={(hover) => hashtagVisual?.anchorProps?.(getHashtagName(linkName), hover)}
    >
