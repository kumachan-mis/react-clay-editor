    ...
    constructor() {
        ...
        this.Indent = (props) => {
            ...
            return (React.createElement("span", { style: constants.style(props.indent.length) },
                [...props.indent].map((char, charIndex) => (React.createElement("span", { key: charIndex, id: TextLinesConstants.char.id(props.lineIndex, charIndex), style: constants.pad.style }, char))),
            ...
            return (React.createElement("span", { style: constants.style(indent.length) },
                textsWithFont.map((textWithFont, withFontIndex) => {
                    ...
                    return (React.createElement("span", { key: withFontIndex, style: style },
                        [...text.substring(0, start)].map((char, charIndex) => (React.createElement("span", { key: charIndex, id: charId(lineIndex, offset + charIndex) }, cursorOn ? char : ""))),
                        [...text.substring(start, end)].map((char, charIndex) => (React.createElement("span", { key: charIndex, id: charId(lineIndex, offset + start + charIndex) }, char))),
                        [...text.substring(end)].map((char, charIndex) => (React.createElement("span", { key: charIndex, id: charId(lineIndex, offset + end + charIndex) }, cursorOn ? char : "")))));
                }),
                React.createElement("span", { id: charId(lineIndex, lineLength) })));
        };
    }
    render() {
        const constants = TextLinesConstants.line;
        return (React.createElement("div", { id: TextLinesConstants.id, style: TextLinesConstants.style }, this.props.text.split("\n").map((line, index) => {
            const { indent, content } = analyzeLine(line);
            return (React.createElement("div", { id: constants.id(index), key: index, style: constants.style(this.props.textStyle.fontSizes.level1) },
