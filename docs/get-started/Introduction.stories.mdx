...
afford to think deeply about typing and formatting of the text since they have to figure out what to write quickly.
...
a feature to shortcut typing. React Clay Editor can show suggestions from given candidates and helps typing of user.

Utilizing already-existing documents is another good way to reduce typing. If rich links to documentations are
available, putting links automatically make your document informative. React Clay Editor enables you to to shorten and
group links with "tags". Since links become shorter, a lot of links does not lose readability of documents. What is
more, since the links can be colorized by tags, users can recognize sources of links at a glance.
...
their hands while editing text. Others would say that they are used to formatting text by selecting text and clicking
buttons. Another example is markdown or new one. Some would say that they love markdown-like experience. Others would
say they are looking for a new editing experience.

To answer these verious needs, React Clay Editor offer some choices. Users can apply format by syntactic writing. At the
same time, users can do this by selecting text and clicking buttons in the syntax menu. The editor toggles between the
view mode and edit the mode in-place. On the other hand, the viewer is available to implement a dual-paned editor.
...
- **in-place mode change**: Like a markdown editor, React Clay Editor has two modes - the view mode and the edit mode. A
  ...
  other lines are stil in the view mode. If users use a markdown editor, they to come and go between these modes by
...
- **suggestions of autocomplete**: Configrations for suggestions of autocomplete are available. If you give candidates
  to the editor, suggestions are shown in normal text, headings and links. In the demo above, when you type just "R",
...
- **tagged links to references**: While learning or working, users would utilize various external resources like files
  ...
  document. To solve this problem, this editor enables them to shorten and group links with "tags". In the demo above,
  `npm` tag and `github` tag are provided. As a result, https://www.npmjs.com/package/react-clay-editor is shorten to
...
- **syntax menu**: Formatting without a mouse is a good way to write documents quickly. However, some users are not
  familiar with such syntactic writing. They may be used to applying formats by menu buttons like Microsoft Word. To
  meet their needs, this package provides the syntax menu. It is a group of buttons and dropdown menus to insert and
  delete the syntactic characters. Users can choose whether they apply formats by typing their keyboard or clicking
  their mouse.
...
- **viewer component**: In-place mode change is a significant feature of React Clay Editor. Having said that, there is
  ...
  a preview of the editor. In <LinkTo kind="Get Started/Formatting Syntax">Formatting Syntax</LinkTo> page, the viewer
  shows the formatting results.

- **multi-components API**: The API consists of multiple components - `<EditorRoot>`, `<EditorSyntaxMenu>`,
  `<EditorTextFieldBody>` and others. It will be simpler when `<Editor>` component is defined for you. However, as we
  described above, needs for a text editor are various and flexibility is highly important. If you don't need the syntax
  menu, you only have to remove `<EditorSyntaxMenu>` from the components. If you want to add your own footer component,
  you only have to add `<YourOwnFooter>` to the components. This is a notable advantage of multi-components API.
